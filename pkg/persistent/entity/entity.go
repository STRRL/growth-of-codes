...
type FileComplexitySnapshot struct {
	gorm.Model
	Project    string    `json:"project" gorm:"column:project;not null;index:idx_project_commit_file,priority:1,unique;index:idx_project_language;"`
	CommitHash string    `json:"commitHash" gorm:"column:commit_hash;not null;index:idx_project_commit_file,priority:2,unique"`
	CommitTime time.Time `json:"commitTime" gorm:"column:commit_time;not null"`
